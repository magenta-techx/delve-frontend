...
function formFieldVariants({
  variant = 'default',
  size = 'default',
}: { variant?: Variant | undefined; size?: Size | undefined } = {}): string {
  const base =
  ...
  const variants: Record<Variant, string> = {
    default:
      'border border-[#D9D9D9] bg-white focus:border-primary focus-visible:border-primary ',
    outline:
      'border border-primary bg-background focus-visible:border-primary focus-visible:ring-primary/50 focus-visible:ring-[3px]',
    ghost:
      'border-0 bg-transparent focus-visible:border-primary focus-visible:ring-primary/50 focus-visible:ring-[3px]',
  };
    ...
    return (
      <div className={cn('flex flex-col gap-1.5', containerClassName)}>
        {label && (
          <label
            className={cn(
              'font-inter text-[0.8rem] font-medium text-[#0F172B] md:text-sm',
              labelClassName
            )}
            htmlFor={label}
          >
