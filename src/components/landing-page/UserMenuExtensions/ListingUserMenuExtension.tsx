...
const ListingUserMenuExtension = ({
  categories,
}: listingUserMenuExtensionProps): JSX.Element => {
  return (
    <div className='mt-8'>
      <h1 className='mb-3 font-inter text-[32px] font-extrabold'>
        Browsers Category
      </h1>
        ...
        <div className='flex items-start'>
          {categories.map((category, key) => {
            return (
              <div
                key={key}
                className='mr-5 w-[200px] border-r-[1px] border-gray-200'
              >
                <h1 className='mb-3 text-sm text-gray-600'>{category.name}</h1>
                <div className='flex flex-col gap-5'>
                  {category.categories.map(subcategory => {
                    ...
                    return (
                      <div key={subcategory.id} className='w-[160px]'>
                        <Link
                          href='/'
