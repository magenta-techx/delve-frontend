...
export function useBusinessCampaignAnalytics(params: {
  businessId?: string | number | undefined;
  requested_metric: 'promotion' | 'advert';
  filter_method?:
    | 'all_time'
    | 'this_month'
    | 'last_6_months'
  ...
  return useQuery<ApiEnvelope<CampaignAnalyticsResponse>, Error>({
    queryKey: [
      ...
      params.businessId,
      params.requested_metric,
      params.filter_method,
    ],
    queryFn: async () => {
      const res = await apiRequest(
        `/api/business/${params.businessId}/analytics/campaigns?${qs.toString()}`
      );
      const data = await res.json();
      if (!res.ok)
        throw new Error(
          data?.message || data?.error || 'Failed to fetch campaign analytics'
        );
      return data;
    },
    ...
    placeholderData: (previousData, previousQuery) => {
      if (previousQuery?.queryKey?.[1] === params.businessId) {
        return previousData;
      }
      return undefined;
    },
...
export function useApprovedBusinesses(): UseQueryResult<
  ApiEnvelope<BusinessSummary[]>,
  Error
> {
  return useQuery({
    queryKey: ['approved-businesses'],
    queryFn: async () => {
      const res = await apiRequest(`/api/business/approved`, {}, undefined, {
        skipAuthRedirect: true,
      });
      const json: PaginatedResponse<BusinessSummary[]> = await res.json();
      if (!res.ok)
        throw new Error(json?.results?.message || 'Failed to fetch businesses');
      return json.results;
    },
    retry: 2,
  });
...
export function useSearchBusinesses(params?: {
...
}): UseQueryResult<ApiEnvelope<BusinessSummary[]>, Error> {
  const qs = new URLSearchParams();
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const res = await apiRequest(
        `/api/business/search?${qs.toString()}`,
        {},
        undefined,
        { skipAuthRedirect: true }
      );
...
export function useTrendingBusiness(): UseQueryResult<
  ApiEnvelope<TrendingBusinessData>,
  Error
> {
  return useQuery({
    queryKey: ['trending-business'],
    queryFn: async () => {
      const res = await apiRequest(`/api/business/trending`, {}, undefined, {
        skipAuthRedirect: true,
      });
      const data = await res.json();
      if (!res.ok)
...
export function useBusinessPublic(
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const res = await apiRequest(
        `/api/business/${businessId}?${qs.toString()}`,
        {},
        undefined,
        { skipAuthRedirect: true }
      );
