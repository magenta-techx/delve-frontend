...
export function useUserChats(): UseQueryResult<
  ApiEnvelope<ChatListItem[]>,
  Error
> {
  return useQuery({
    queryKey: ['user-chats'],
...
): UseQueryResult<ApiEnvelope<ChatListItem[]>, Error> {
  return useQuery({
...
): UseQueryResult<ApiEnvelope<ChatMessage[]>, Error> {
  return useQuery({
...
export function useDeleteChatMessages(): UseMutationResult<
  ApiMessage,
  Error,
...
> {
  return useMutation({
    mutationFn: async ({ chat_id }) => {
      const res = await apiRequest(`/api/chat/${chat_id}/delete`, {
        method: 'DELETE',
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok)
        throw new Error((data as any)?.error || 'Delete chat failed');
      return data;
    },
  });
}
