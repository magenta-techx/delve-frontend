...
export default function ListingCardSkeleton({
  classStyle = '',
  group = false,
}: ListingCardSkeletonProps): JSX.Element {
  return (
    <div className='rounded-2xl border-[2px] border-[#FEC601] p-1 md:rounded-3xl'>
      <div
        className={`relative flex ${classStyle} ${group ? 'group' : ''} flex-col items-center justify-end overflow-hidden rounded-2xl font-inter`}
      >
        ...
        <div className='relative z-10 w-full bg-gradient-to-t from-black/70 via-black/30 to-transparent p-4 text-white'>
          <div className='flex items-center gap-2 border-b border-white/50 pb-2 sm:h-[85px] sm:items-start'>
            {/* Logo circle */}
            <div className='mt-2 h-10 w-14 flex-shrink-0 sm:mt-0 sm:h-14 sm:w-14'>
              <div className='h-full w-10 rounded-full bg-white/60 sm:w-14' />
            </div>
