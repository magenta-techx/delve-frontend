...
interface BusinessDetailsGalleryCarouselProps {
  images: (
    | string
    | { id: number; url: string; is_thumbnail: boolean; uploaded_at: string }
  )[];
  initialIndex: number;
  isOpen: boolean;
  onClose: () => void;
...
export function BusinessDetailsGalleryCarousel({
  images,
  initialIndex,
  isOpen,
  onClose,
}: BusinessDetailsGalleryCarouselProps) {
  ...
  return (
    <div
      className='fixed inset-0 z-[700] flex items-center justify-center bg-black bg-opacity-30 backdrop-blur-lg'
      onClick={onClose}
    >
      {/* Close button */}
      <button
        onClick={onClose}
        className='absolute right-6 top-6 z-50 rounded-full bg-black/30 p-2 transition-colors hover:bg-black/50'
          ...
          {images.slice(thumbnailStart, thumbnailEnd).map((image, i) => {
            ...
            return (
              <button
                ...
                className={`2 relative h-16 w-16 flex-shrink-0 overflow-hidden rounded-lg transition-all`}
