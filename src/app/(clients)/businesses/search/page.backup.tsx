...
const FilterContent = ({
  ...
  filteredLocations: Array<{ name: string }>;
  filteredCategories: Array<{ name: string; id?: number | string }>;
  locationParam: string | null;
  categoryParam: string | null;
  applyFilters: (patch: SearchPatch) => void;
  handleClearFilters: () => void;
  normalizedFilterSearch: string;
  categoriesData: { data: Array<{ name: string; id?: number | string }> } | undefined;
  onClose?: () => void;
}): JSX.Element => {
  const getCategoryIcon = (categoryName: string) => {
    const iconKey = categoryName.split(' ')[0].toLowerCase();
    return iconKey;
  };
      ...
      <section className=''>
        <div className='border-t border-[#E3E8EF] pt-2'>
          <button
            ...
            className='flex w-full items-center justify-between py-3 text-left text-sm font-medium text-[#64748B]'
          >
            <span>By States</span>
            <svg
              width='12'
          ...
          <div
            className={cn(
              'space-y-2 text-sm transition-all duration-200',
              openSections.cities
          ...
          >
            ...
            {filteredLocations.map(state => {
              ...
              return (
                <button
                  ...
                  type='button'
                  onClick={() => {
                    applyFilters({ locationName: name });
                    onClose?.();
                  }}
                  className={cn(
                    'w-full rounded-xl px-3 py-2 text-left transition',
            ...
            {normalizedFilterSearch && filteredLocations.length === 0 ? (
              <p className='rounded-xl bg-[#F8FAFC] px-3 py-2 text-xs text-[#64748B]'>
                No states match "{filterSearchTerm}".
              </p>
            ) : null}
          </div>
        </div>
        ...
        <div className='border-t border-[#E3E8EF] pt-2'>
          ...
          >
            {filteredCategories.map(category => {
              ...
              const iconKey = getCategoryIcon(name);
              ...
              return (
                <button
                  ...
                  type='button'
                  onClick={() => {
                    applyFilters({ categoryName: name });
                    onClose?.();
                  }}
                  className={cn(
...
const BusinessSearchPage = (): JSX.Element => {
  ...
  return (
    <div className='relative mx-auto flex min-h-screen w-full max-w-7xl flex-1 flex-col gap-10 bg-[#F8FAFC] px-4 pb-24 pt-32 lg:flex-row lg:px-6'>
      <aside className='top-24 h-fit w-full shadow-sm lg:sticky lg:w-72 lg:flex-shrink-0'>
        <div className='flex flex-col gap-6'>
          <div className='space-y-4'>
            <h2 className='text-base font-semibold text-[#0F172B]'>
              Filter Search
