...
export function useSubcategories(categoryId: number | null): UseQueryResult<Subcategory[], Error> {
  return useQuery({
    queryKey: ["subcategories", categoryId],
    queryFn: async () => {
      if (!categoryId) return [];
      const res = await authAwareFetch(`/api/business/subcategories?category_id=${categoryId}`);
      const data = await res.json();
...
export function useBusinessPerformance(businessId: BusinessId, params?: { filter?: string; metric?: string }): UseQueryResult<ApiEnvelope<BusinessPerformanceData>, Error> {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const res = await authAwareFetch(`/api/business/${businessId}/performance?${qs.toString()}`);
      const data = await res.json();
      if (!res.ok) throw new Error(data?.error || "Failed to fetch performance");
      return data;
    },
    enabled: Boolean(businessId),
  });
...
export function useBusinessActivation(): UseMutationResult<ApiMessage, Error, { business_id: BusinessId; activation_status: "activate" | "deactivate"; business_name?: string; reason_for_deactivation?: string }> {
  ...
  return useMutation<ApiMessage, Error, { business_id: BusinessId; activation_status: "activate" | "deactivate"; business_name?: string; reason_for_deactivation?: string }>({
    mutationFn: async (payload) => {
      ...
      const res = await authAwareFetch(`/api/business/${business_id}/activation`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
...
export function useUploadBusinessImages(): UseMutationResult<ApiEnvelope<string[]>, Error, { business_id: BusinessId; images: FileList | File[] }> {
  ...
  return useMutation<ApiEnvelope<string[]>, Error, { business_id: BusinessId; images: FileList | File[] }>({
    mutationFn: async ({ business_id, images }) => {
      ...
      const res = await authAwareFetch(`/api/business/${business_id}/upload-image`, {
        method: "POST",
        body: fd,
      });
      const data = await res.json();
...
export function useDeleteBusinessImages(): UseMutationResult<ApiMessage, Error, { image_ids: number[] }> {
  ...
  return useMutation<ApiMessage, Error, { image_ids: number[] }>({
    mutationFn: async ({ image_ids }) => {
      ...
      if (!res.ok) throw new Error((data as any)?.error || "Delete image failed");
      return data;
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["business"] });
