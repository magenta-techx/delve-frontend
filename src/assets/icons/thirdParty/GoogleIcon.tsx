'use client';
import React from 'react';

const GoogleIcon = (): JSX.Element => {
  return (
    <svg
      width='20'
      height='20'
      viewBox='0 0 25 24'
      fill='none'
      xmlns='http://www.w3.org/2000/svg'
    >
      <rect width='24' height='24' transform='translate(0.5)' fill='' />
      <path
        fillRule='evenodd'
        clipRule='evenodd'
        d='M23.54 12.2605C23.54 11.445 23.4668 10.6609 23.3309 9.9082H12.5V14.3566H18.6891C18.4225 15.7941 17.6123 17.0121 16.3943 17.8275V20.713H20.1109C22.2855 18.7109 23.54 15.7627 23.54 12.2605Z'
        fill='#4285F4'
      />
      <path
        fillRule='evenodd'
        clipRule='evenodd'
        d='M12.5 23.5005C15.605 23.5005 18.2081 22.4708 20.1109 20.7144L16.3943 17.829C15.3645 18.519 14.0472 18.9267 12.5 18.9267C9.50474 18.9267 6.96951 16.9037 6.06519 14.1855H2.22314V17.1651C4.11542 20.9235 8.00451 23.5005 12.5 23.5005Z'
        fill='#34A853'
      />
      <path
        fillRule='evenodd'
        clipRule='evenodd'
        d='M6.06523 14.1855C5.83523 13.4955 5.70455 12.7584 5.70455 12.0005C5.70455 11.2425 5.83523 10.5055 6.06523 9.81548V6.83594H2.22318C1.44432 8.38844 1 10.1448 1 12.0005C1 13.8562 1.44432 15.6125 2.22318 17.165L6.06523 14.1855Z'
        fill='#FBBC05'
      />
      <path
        fillRule='evenodd'
        clipRule='evenodd'
        d='M12.5 5.07386C14.1884 5.07386 15.7043 5.65409 16.8961 6.79364L20.1945 3.49523C18.2029 1.63955 15.5997 0.5 12.5 0.5C8.00451 0.5 4.11542 3.07705 2.22314 6.83545L6.06519 9.815C6.96951 7.09682 9.50474 5.07386 12.5 5.07386Z'
        fill='#EA4335'
      />
    </svg>
  );
};

export default GoogleIcon;
